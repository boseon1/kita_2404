...
def translate_keyword_g(keyword, target_language='en'):
    ...
    print('구글 translation.text  == ' + translation.text)
...
def translate_keyword_d(keyword):
    ...
    print('deepl result.text  == ' + result.text)
    return result.text
...
def translate_keyword_n(text):
    ...
    if response.status_code == 200:
        ...
        print('네이버 translated_text  == ' + translated_text)
            ...
            if filter_type == 'patent':
                ...
                for field in application_fields:
                    for keyword in categories.get(field, []):
                        paper_conditions.append((df_papers['title'].str.lower().contains(keyword, na=False)) | 
                                                (df_papers['Abstract'].str.lower().contains(keyword, na=False)))
                if paper_conditions:
